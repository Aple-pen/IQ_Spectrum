...
TcpSender::TcpSender() : listenSocket_(InvalidSocketValue), clientSocket_(InvalidSocketValue) {
    WSADATA data{};
...
bool TcpSender::StartListening(const std::string &bindIp, uint16_t port, std::string &error) {
    ...
    BOOL reuse = TRUE;
    if (setsockopt(listenSock, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<const char *>(&reuse), sizeof(reuse)) ==
        SOCKET_ERROR) {
...
bool TcpSender::TryRecvChunkRequest(int firstByteTimeoutMs, int totalTimeoutMs, int &chunkBytes, std::string &error) {
    if (!IsConnected()) {
    ...
    while (received < request.size()) {
        ...
        const int got = ::recv(static_cast<SOCKET>(clientSocket_), reinterpret_cast<char *>(request.data() + received),
    ...
    if (requestedChunkBytes == 0 || requestedChunkBytes > static_cast<uint32_t>(INT_MAX)) {
...
bool TcpSender::IsConnected() const { return clientSocket_ != InvalidSocketValue; }