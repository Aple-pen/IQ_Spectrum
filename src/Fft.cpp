...
bool Fft::IsPowerOfTwo(int value) { return value > 0 && (value & (value - 1)) == 0; }
...
std::vector<float> Fft::MagnitudeSpectrumIQ(const std::vector<float>& iSamples, const std::vector<float>& qSamples,
                                            int fftSize, float sampleRateHz, float freqOffsetHz) {
    if (!IsPowerOfTwo(fftSize) || static_cast<int>(iSamples.size()) < fftSize ||
