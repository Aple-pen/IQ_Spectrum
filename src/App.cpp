...
const char *FrequencyName(int index) {
    static const char *kNames[] = {"915 MHz",      "433 MHz",     "5.8 GHz_Low", "5.8 GHz_Mid",
                                   "5.8 GHz_High", "2.4 GHz Low", "2.4 GHz High"};
    if (index < 0 || index >= 7) {
        return "unknown";
    }
    return kNames[index];
...
void App::LoadSettings(const char *path) {
    ...
    while (std::getline(f, line)) {
        ...
        if (key == "filePath") {
            ...
            strncpy_s(serverIp_.data(), serverIp_.size(), val.c_str(), serverIp_.size() - 1);
...
IqStream &App::Active() {
    return mode_ == static_cast<int>(Mode::Receive) ? static_cast<IqStream &>(receiver_)
...
void App::RenderControls(const StreamSnapshot &snapshot) {
    ...
    ImGui::RadioButton("Playback (Server)", &mode_, static_cast<int>(Mode::Playback));
    ImGui::SameLine();
    ImGui::RadioButton("Receive (Client)", &mode_, static_cast<int>(Mode::Receive));
    ImGui::EndDisabled();
    ...
    if (!receiveMode) {
        ...
        static const port::FrequencyValues FrequencyValues[] = {
            port::FrequencyValues::FREQ_915_MHZ,      port::FrequencyValues::FREQ_433_MHZ,
            port::FrequencyValues::FREQ_5_8_GHZ_LOW,  port::FrequencyValues::FREQ_5_8_GHZ_MID,
            port::FrequencyValues::FREQ_5_8_GHZ_HIGH, port::FrequencyValues::FREQ_2_4_GHZ_LOW,
        ...
        if (sampleRateHz_ > 0.0f) {
            const float fftWindowMs = 1000.0f * static_cast<float>(fftSize_) / sampleRateHz_;
    ...
    ImGui::TextColored(accent, "FFT");
    {
        ...
        if (ImGui::BeginCombo("FFT size", kFftLabels[curIdx])) {
            for (int i = 0; i < kFftCount; ++i) {
                ...
                if (ImGui::Selectable(kFftLabels[i], selected)) fftSize_ = kFftSizes[i];
        ...
        float rbw = (sampleRateHz_ > 0.0f && fftSize_ > 0) ? sampleRateHz_ / static_cast<float>(fftSize_) : 0.0f;
    ...
    if (ImGui::DragFloat("##freqOffset", &freqOffsetHz_,
                         sampleRateHz_ > 0 ? sampleRateHz_ / static_cast<float>(fftSize_) : 1.0f, -sampleRateHz_ * 0.5f,
                         sampleRateHz_ * 0.5f, "%.1f Hz")) {
    ...
    if (!snapshot.streamRunning) {
        if (ImGui::Button("Start", ImVec2(110, 34))) {
            std::string error;
            const bool ok = receiveMode ? receiver_.Start(BuildConfig(), error) : streamer_.Start(BuildConfig(), error);
            if (!ok) {
                lastError_ = error;
            }
        }
...
void App::RenderSpectrum(const StreamSnapshot &snapshot, float width) {
    ...
    {
        const ImVec4 specAccent =
            chartDark_ ? ImVec4(1.000f, 0.792f, 0.188f, 1.0f) : ImVec4(0.169f, 0.424f, 0.690f, 1.0f);
    ...
    if (ImPlot::BeginPlot("##spectrum", plotSize)) {
        ...
        if (!snapshot.frequencies.empty() && snapshot.frequencies.size() == snapshot.magnitudesDb.size()) {
            ...
            ImPlot::PushStyleColor(ImPlotCol_Line, chartDark_ ? ImVec4(1.000f, 0.792f, 0.188f, 1.0f)
                                                              : ImVec4(0.169f, 0.424f, 0.690f, 1.0f));
            ImPlot::PlotLine("Magnitude", snapshot.frequencies.data(), snapshot.magnitudesDb.data(),
    ...
    if (showSpectrogram_ && !snapshot.magnitudesDb.empty()) {
        ...
        if (bins != spectrogramBins_ || static_cast<int>(spectrogramBuf_.size()) != spectrogramRows_ * bins) {
        ...
        if (spectrogramFill_ > 0) {
            ...
            for (int r = 0; r < spectrogramFill_; ++r) {
                ...
                std::copy_n(spectrogramBuf_.begin() + srcRow * bins, bins, spectrogramDisp_.begin() + r * bins);
            }
            ...
            const double xMin2 = snapshot.frequencies.empty() ? 0.0 : static_cast<double>(snapshot.frequencies.front());
            const double xMax2 = snapshot.frequencies.empty() ? 1.0 : static_cast<double>(snapshot.frequencies.back());
            ...
            if (ImPlot::BeginPlot("##spectrogram", spectroSize)) {
                ...
                ImPlot::SetupAxisLimits(ImAxis_Y1, 0.0, static_cast<double>(spectrogramRows_), ImGuiCond_Always);
                // rows=spectrogramRows_, cols=bins
                // bounds: (xMin2,yMin)=(freq_start,0) → (xMax2, spectrogramRows_)
                ImPlot::PlotHeatmap("##heatmap", spectrogramDisp_.data(), spectrogramRows_, bins,
                                    static_cast<double>(scaleMin), static_cast<double>(scaleMax), nullptr,
                                    ImPlotPoint(xMin2, 0.0), ImPlotPoint(xMax2, static_cast<double>(spectrogramRows_)));
                ImPlot::EndPlot();
            }
            ImPlot::PopColormap();
            ImPlot::PopStyleColor(5);
...
void App::RenderConstellation(const StreamSnapshot &snapshot) {
    ...
    const bool hasData = !snapshot.iSamples.empty() && snapshot.iSamples.size() == snapshot.qSamples.size();
    ...
    if (ImPlot::BeginPlot("##constellation", ImVec2(-1, -1), ImPlotFlags_Equal)) {
        ...
        if (hasData) {
            ...
            ImPlot::PushStyleColor(ImPlotCol_MarkerFill, chartDark_ ? ImVec4(1.000f, 0.792f, 0.188f, 0.7f)
            ...
            ImPlot::PlotScatter("IQ", snapshot.iSamples.data() + offset, snapshot.qSamples.data() + offset, count);
