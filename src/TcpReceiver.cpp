...
bool TcpReceiver::Connect(const std::string &ip, uint16_t port, int timeoutMs, std::string &error) {
    Close();
...
bool TcpReceiver::SendAll(const uint8_t *bytes, size_t count, std::string &error) {
    if (!IsConnected()) {
        error = "Not connected";
        return false;
    }
    ...
    while (remaining > 0) {
        const int want = static_cast<int>(remaining > static_cast<size_t>(INT_MAX) ? INT_MAX : remaining);
...
int TcpReceiver::Recv(uint8_t *buffer, int maxBytes, int timeoutMs, std::string &error) {
    if (!IsConnected()) {
        error = "Not connected";
        return -1;
    }
    ...
    const int received = ::recv(static_cast<SOCKET>(socket_), reinterpret_cast<char *>(buffer), maxBytes, 0);
