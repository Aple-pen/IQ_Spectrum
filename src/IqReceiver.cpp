...
bool IqReceiver::RecvExact(TcpReceiver &client, uint8_t *dst, size_t n, std::string &error) {
    size_t got = 0;
    while (got < n) {
        if (stopRequested_) {
        ...
        const int want = static_cast<int>(std::min<size_t>(n - got, static_cast<size_t>(INT_MAX)));
...
void IqReceiver::Run(StreamConfig config) {
    ...
    const size_t         minChunkBytes = static_cast<size_t>(std::max(config.chunkBytes, 1));
    ...
    while (!stopRequested_) {
        ...
        while (!stopRequested_) {
            ...
            fftPendingBytes.insert(fftPendingBytes.end(), payload.begin(), payload.end());
