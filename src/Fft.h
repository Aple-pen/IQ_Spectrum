...
class Fft {
   public:
    ...
    static std::vector<float> MagnitudeSpectrumIQ(const std::vector<float>& iSamples,
                                                  const std::vector<float>& qSamples, int fftSize, float sampleRateHz,
                                                  float freqOffsetHz = 0.0f);
