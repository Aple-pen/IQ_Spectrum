...
public:
  ...
  bool Connect(const std::string &ip, uint16_t port, int timeoutMs,
               std::string &error);
  // 정확히 count 바이트를 서버로 전송.
  bool SendAll(const uint8_t *bytes, size_t count, std::string &error);
  // 최대 maxBytes 바이트 수신. 반환값: >0 수신 바이트, 0 연결 종료, <0
  // 오류(error 설정). timeoutMs 동안 데이터가 없으면 0을 반환하지 않고 다시
  // 기다린다(블로킹). -2 = 타임아웃(중단 확인용).
